...
final class ServerUsersViewModel: ViewModel, Eventful, Stateful, Identifiable {
    ...
    private func refreshUser(_ userID: String) async throws {
        let newUser: UserDto = try await userSession.embyClient.user(
            userID: userID,
            as: UserDto.self
        )
    ...
    private func loadUsers(isHidden: Bool, isDisabled: Bool) async throws {
        ...
        if isHidden {
            queryItems.append(URLQueryItem(name: "IsHidden", value: "true"))
        }
        if isDisabled {
            queryItems.append(URLQueryItem(name: "IsDisabled", value: "true"))
        }

        let response: [UserDto] = try await userSession.embyClient.users(
    ...
    private func deleteUser(id: String) async throws {
        try await userSession.embyClient.deleteUser(userID: id)
    }
