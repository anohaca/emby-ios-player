...
extension BaseItemDto {
    ...
    func getNowPlayingImage() async -> UIImage? {
        ...
        guard let firstImage = await ImagePipeline.Emby.other.loadFirstImage(from: imageSources) else {
...
//                .appendingPathComponent(id)
    ...
    func getFullItem(userSession: UserSession, sendNotification: Bool = false) async throws -> BaseItemDto {
        ...
        let response: BaseItemDto = try await userSession.embyClient.item(
            itemID: id,
            as: BaseItemDto.self
        )
        ...
        if sendNotification {
            Notifications[.itemMetadataDidChange].post(response)
        }
