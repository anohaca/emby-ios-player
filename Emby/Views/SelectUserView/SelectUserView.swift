...
struct SelectUserView: View {
    ...
    private func requirePasswordSignIn(for user: UserState) {
        guard let server = viewModel.servers.keys.first(where: { $0.id == user.serverID }) else {
            viewModel.error(ErrorMessage("Server missing for \(user.username)."))
            return
        }
        ...
        VStack(spacing: 0) {
            ZStack {
                ...
                case .grid:
                    ...
                    .scrollIfLargerThanContainer(padding: 80, alignment: UIDevice.isPhone ? .top : .center)
                case .list:
                    listContentView
                }
            }
    ...
    @ViewBuilder
    private var editingDeleteBar: some View {
        HStack {
            Spacer()
    ...
    private var connectToServerView: some View {
        VStack(spacing: 10) {
            Text(L10n.connectToEmbyServerStart)
    ...
    var body: some View {
        ...
        .onReceive(viewModel.events) { event in
            switch event {
            case let .needsPasswordSignIn(user):
                requirePasswordSignIn(for: user)
