...
final class IdentifyItemViewModel: ViewModel {
    ...
    private func __actuallySearch(_ query: SearchQuery) async throws {
        ...
        case .boxSet:
            ...
            let response: [RemoteSearchResult] = try await userSession.embyClient.remoteSearchResults(
                itemType: "BoxSet",
                body: parameters,
                as: [RemoteSearchResult].self
            )
    ...
    private func _update(_ searchResult: RemoteSearchResult) async throws {
        ...
        try await userSession.embyClient.applyRemoteSearchResult(itemID: itemID, body: searchResult)
