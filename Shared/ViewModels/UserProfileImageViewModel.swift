...
final class UserProfileImageViewModel: ViewModel, Eventful, Stateful {
    ...
    private func upload(_ image: UIImage) async throws {
        ...
        try await userSession.embyClient.uploadUserImage(
            userID: userID,
    ...
    private func delete() async throws {
        ...
        try await userSession.embyClient.deleteUserImage(userID: userID)
    ...
    private func sweepProfileImageCache() {
        if let userImageURL = user.profileImageSource(client: userSession.embyClient, maxWidth: 60).url {
            ImagePipeline.Emby.local.removeItem(for: userImageURL)
            ImagePipeline.Emby.posters.removeItem(for: userImageURL)
        }

        if let userImageURL = user.profileImageSource(client: userSession.embyClient, maxWidth: 120).url {
            ImagePipeline.Emby.local.removeItem(for: userImageURL)
            ImagePipeline.Emby.posters.removeItem(for: userImageURL)
        }

        if let userImageURL = user.profileImageSource(client: userSession.embyClient, maxWidth: 150).url {
            ImagePipeline.Emby.local.removeItem(for: userImageURL)
            ImagePipeline.Emby.posters.removeItem(for: userImageURL)
        }
    }
