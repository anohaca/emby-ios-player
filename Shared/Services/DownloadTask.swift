...
class DownloadTask: NSObject, ObservableObject {
    ...
    private func downloadMedia() async throws {
        ...
        let response = try await userSession.embyClient.download(
            path: "/Items/\(item.id!)/Download",
            delegate: self
        )
    ...
    private func downloadBackdropImage() async {
        ...
        guard let response = try? await userSession.embyClient.download(
            from: imageURL,
            delegate: self
        ) else { return }
    ...
    private func saveImage(from response: EmbyPortDownloadResponse?, filename: String) {
    ...
    private func getImageFilename(from response: EmbyPortDownloadResponse, secondary: String) -> String {
