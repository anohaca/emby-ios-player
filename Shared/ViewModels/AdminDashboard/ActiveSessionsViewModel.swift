...
final class ActiveSessionsViewModel: ViewModel {
    ...
    private func _refresh() async throws {
        var parameters = EmbyPortSessionsParameters()
        parameters.activeWithinSeconds = activeWithinSeconds

        let response: [SessionInfoDto] = try await userSession.embyClient.sessions(
            queryItems: EmbyPortQueryItemBuilder.queryItems(from: parameters),
            as: [SessionInfoDto].self
        )
        ...
        case .all:
            response
        case .active:
            response.filter { $0.nowPlayingItem != nil }
        case .inactive:
            response.filter { $0.nowPlayingItem == nil }
        }
