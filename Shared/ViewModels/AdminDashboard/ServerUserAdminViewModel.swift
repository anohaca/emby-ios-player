...
final class ServerUserAdminViewModel: ViewModel, Eventful, Stateful, Identifiable {
    ...
    private func loadDetails() async throws {
        ...
        let response: UserDto = try await userSession.embyClient.user(
            userID: userID,
            as: UserDto.self
        )
    ...
    private func loadLibraries(isHidden: Bool?) async throws {
        let queryItems = isHidden.map {
            [URLQueryItem(name: "IsHidden", value: $0 ? "true" : "false")]
        } ?? []
        let response: EmbyPortItemsResponse<BaseItemDto> = try await userSession.embyClient.mediaFolders(
    ...
    private func updatePolicy(policy: UserPolicy) async throws {
        guard let userID = user.id else { throw ErrorMessage("User ID is missing") }
        try await userSession.embyClient.updateUserPolicy(userID: userID, body: policy)
    ...
    private func updateConfiguration(configuration: UserConfiguration) async throws {
        guard let userID = user.id else { throw ErrorMessage("User ID is missing") }
        try await userSession.embyClient.updateUserConfiguration(userID: userID, body: configuration)
    ...
    private func updateUsername(username: String) async throws {
        ...
        try await userSession.embyClient.updateUser(userID: userID, body: updatedUser)
