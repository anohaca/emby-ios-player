...
struct SelectorView<Element: Hashable, Label: View>: View {
    ...
    private let externalSelection: Binding<Set<Element>>
    ...
    init(
        ...
        let setSelection = selection.map(
            getter: { Set($0) },
            setter: { Array($0) }
        )
        ...
        self.externalSelection = setSelection
    ...
    private func syncExternalSelection(_ selection: Set<Element>) {
        guard box.value != selection else { return }
        box.value = selection
    }

    var body: some View {
        ForEach(sources, id: \.hashValue) { element in
            Button {
                switch type {
                case .single:
                    box.value = [element]
            ...
            .buttonStyle(.plain)
            .listRowBackground(Color.embyAppBackgroundSurface)
        }
        .onAppear {
            syncExternalSelection(externalSelection.wrappedValue)
        }
        .onChange(of: externalSelection.wrappedValue) { newValue in
            syncExternalSelection(newValue)
        }
    }
}
