...
final class SQLObservable<Value: Storable>: ObservableObject, _StoredValueObservable {
    ...
    private func makeObjectPublisher() -> ObjectPublisher<AnyStoredData>? {
        ...
        if let values = try? EmbyStore.dataStack.fetchAll(clause), let first = values.first {
            let publisher = first.asPublisher(in: EmbyStore.dataStack)
            ...
            do {
                ...
                Logger.emby().error("Unable to store and create publisher for: \(key)")
