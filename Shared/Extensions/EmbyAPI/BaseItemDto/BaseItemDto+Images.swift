...
extension BaseItemDto {
    ...
    func _imageURL(
        ...
        itemID: String,
        requireTag: Bool = true,
        tag overrideTag: String? = nil
    ) -> URL? {
        let scaleWidth = maxWidth.map { Double(UIScreen.main.scale($0)) }
        let scaleHeight = maxHeight.map { Double(UIScreen.main.scale($0)) }
        let validQuality = quality.map { clamp($0, min: 1, max: 100) }

        let tag = overrideTag ?? getImageTag(for: type)

        guard tag != nil || !requireTag else { return nil }

        guard let client = Container.shared.currentUserSession()?.embyClient else { return nil }

        return client.itemImageURL(
            itemID: itemID,
