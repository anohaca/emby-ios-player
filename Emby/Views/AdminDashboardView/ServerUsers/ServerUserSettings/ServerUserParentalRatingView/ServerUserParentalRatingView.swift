...
struct ServerUserParentalRatingView: View {
    ...
    private func reducedParentalRatings() -> [EmbyParentalRating] {
        [EmbyParentalRating(name: L10n.none, value: nil)] +
            parentalRatingsViewModel.value.grouped { $0.value ?? 0 }
            .map { key, group in
                if key < 100 {
                    if key == 0 {
                        return EmbyParentalRating(name: L10n.allAudiences, value: key)
                    } else {
                        return EmbyParentalRating(name: L10n.agesGroup(key), value: key)
                    }
                } else {
                    let name = group
                        .compactMap(\.name)
                    ...
                    return EmbyParentalRating(name: name, value: key)
                }
            }
            .sorted(using: \.value)
    }
