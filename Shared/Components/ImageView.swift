...
struct ImageView<Failure: View>: View {
    ...
    private let requestedSources: [ImageSource]
    ...
    private init(
        ...
        placeholder: ((ImageSource) -> any View)?,
        failure: Failure
    ) {
        self.requestedSources = sources
        self._sources = State(initialValue: sources)
        self.image = image
    ...
    var body: some View {
        ...
        .onChange(of: requestedSources) { newSources in
