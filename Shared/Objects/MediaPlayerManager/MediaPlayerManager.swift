...
//    }
    ...
    init(
        ...
        let resolvedQueue = Self.resolvedQueue(for: item, explicitQueue: queue)
        self.item = item
        self.queue = resolvedQueue.map { AnyMediaPlayerQueue($0) }
    ...
    init(
        ...
        let resolvedQueue = Self.resolvedQueue(for: playbackItem.baseItem, explicitQueue: queue)
        self.item = playbackItem.baseItem
        self.queue = resolvedQueue.map { AnyMediaPlayerQueue($0) }
        self.state = .playback
        super.init()
    ...
    private static func resolvedQueue(
        for item: BaseItemDto,
        explicitQueue: (any MediaPlayerQueue)?
    ) -> (any MediaPlayerQueue)? {
    ...
    private func _ended() async throws {
        ...
        // Some player proxies may send ended early.
