...
class PagingLibraryViewModel<Element: Poster>: ViewModel, Eventful, Stateful {
        ...
        case .refresh:
            ...
            pagingTask = Task { [weak self] in
                ...
                do {
                    ...
                    await MainActor.run {
                        self.backgroundStates.remove(.refresh)
                        self.logger.error("Paging refresh failed: \(error.embyDiagnosticDescription)")
        ...
        case .getNextPage:
            ...
            pagingTask = Task { [weak self] in
                do {
                    ...
                    await MainActor.run {
                        self?.backgroundStates.remove(.gettingNextPage)
                        self?.logger.error("Paging next page failed: \(error.embyDiagnosticDescription)")
                        self?.state = .error(.init(error.embyDisplayDescription))
                    }
                }
            }
            .asAnyCancellable()
    ...
    final func refresh() async throws {
        ...
        let refreshedElements = IdentifiedArray(
            pageItems,
...
extension PagingLibraryViewModel where Element == BaseItemDto {

    func addingChildImageFallbacks(to items: [BaseItemDto]) async -> [BaseItemDto] {
        var resolvedItems: [BaseItemDto] = []
        resolvedItems.reserveCapacity(items.count)

        for item in items {
            guard item.needsChildImageFallback,
                  let fallback = try? await firstChildImageFallback(for: item) else {
                resolvedItems.append(item)
                continue
            }

            resolvedItems.append(fallback)
        }
    ...
    private func firstChildImageFallback(for item: BaseItemDto) async throws -> BaseItemDto? {
        ...
        guard let child = response.items?.first(where: { $0.imageTags?[ImageType.primary.rawValue] != nil }),
              ...
              let primaryImageTag = child.imageTags?[ImageType.primary.rawValue] else {
            return nil
        }

        return item
            .mutating(\.parentPrimaryImageItemID, with: childID)
            .mutating(\.parentPrimaryImageTag, with: primaryImageTag)
    }
}

private extension BaseItemDto {

    var needsChildImageFallback: Bool {
