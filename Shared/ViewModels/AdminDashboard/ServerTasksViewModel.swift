...
final class ServerTasksViewModel: ViewModel {
    ...
    private func _refresh() async throws {
        let allTasks: [EmbyTaskInfo] = try await userSession.embyClient.tasks(
            queryItems: [
                URLQueryItem(name: "IsHidden", value: "false"),
                URLQueryItem(name: "IsEnabled", value: "true")
            ],
    ...
    private func _restartApplication() async throws {
        try await userSession.embyClient.restartApplication()
    }

    @Function(\Action.Cases.shutdownApplication)
    private func _shutdownApplication() async throws {
        try await userSession.embyClient.shutdownApplication()
    }
}
