...
final class GenreEditorViewModel: ItemEditorViewModel<String> {
    ...
    override func searchElements(_ searchTerm: String) async throws -> [String] {
        let parameters = EmbyPortGenresParameters(searchTerm: searchTerm.isEmpty ? nil : searchTerm)
        let response: EmbyPortItemsResponse<BaseItemDto> = try await userSession.embyClient.genres(
            parameters,
            as: EmbyPortItemsResponse<BaseItemDto>.self
        )

        return response.items?.compactMap(\.name) ?? []
    }
