...
class EpisodeMediaPlayerQueue: ViewModel, MediaPlayerQueue {
    ...
    private func getAdjacentEpisodes(for item: BaseItemDto?) async throws {
        ...
        let previousIndex = currentIndex > items.startIndex ? items.index(before: currentIndex) : nil
        let nextIndex = currentIndex < items.index(before: items.endIndex) ? items.index(after: currentIndex) : nil
        let previousItem = previousIndex.map { items[$0] }
