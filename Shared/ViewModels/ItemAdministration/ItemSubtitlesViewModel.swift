...
final class ItemSubtitlesViewModel: ViewModel {
    ...
    private func __actuallySearch(_ isPerfectMatch: Bool) async throws {
        ...
        let results: [RemoteSubtitleInfo] = try await userSession.embyClient.searchRemoteSubtitles(
            itemID: itemID,
            language: language,
            isPerfectMatch: isPerfectMatch,
            as: [RemoteSubtitleInfo].self
        )
    ...
    private func _set(_ subtitles: Set<String>) async throws {
        ...
        try await withThrowingTaskGroup(of: Void.self) { group in
            for subtitleID in subtitles {
                group.addTask {
                    try await self.userSession.embyClient.downloadRemoteSubtitle(
                        itemID: itemID,
    ...
    private func _upload(_ file: URL, _ isForced: Bool, _ isHearingImpaired: Bool) async throws {
        ...
        try await userSession.embyClient.uploadSubtitle(itemID: itemID, body: subtitle)
    ...
    private func _delete(_ mediaStreams: Set<MediaStream>) async throws {
        ...
        for index in indices {
            do {
                try await userSession.embyClient.deleteSubtitle(itemID: itemID, index: index)
            } catch {
