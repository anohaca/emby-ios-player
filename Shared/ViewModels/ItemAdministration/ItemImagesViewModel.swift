...
final class ItemImagesViewModel: ViewModel, Stateful, Eventful {
    ...
    private func getAllImages() async throws {
        ...
        let response: [ImageInfo] = try await self.userSession.embyClient.itemImageInfos(
            itemID: itemID,
            as: [ImageInfo].self
        )

        let newImages: [ImageType: [ImageInfo]] = response.grouped(by: \.imageType)
    ...
    private func setImage(_ remoteImageInfo: RemoteImageInfo) async throws {
        ...
        try await userSession.embyClient.downloadRemoteImage(
            itemID: itemID,
            imageType: type,
            imageURL: imageURL
        )
    ...
    private func upload(imageData: Data, imageType: ImageType, contentType: String) async throws {
        ...
        try await userSession.embyClient.setItemImage(
            itemID: itemID,
    ...
    private func deleteImage(_ imageInfo: ImageInfo) async throws {
        ...
        try await userSession.embyClient.deleteItemImage(
            itemID: itemID,
            imageType: imageType.rawValue,
