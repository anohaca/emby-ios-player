...
extension ImagePipeline.Emby {
    ...
    static let posters: ImagePipeline = ImagePipeline(delegate: EmbyImagePipelineDelegate()) { config in
        config.dataCache = DataCache.Emby.posters

        let dataLoader = DataLoader(
            configuration: .emby
        )
        dataLoader.delegate = URLSessionProxyDelegate(
            logger: NetworkLogger.emby(),
            delegate: nil
        )
    ...
    static let local: ImagePipeline = ImagePipeline(delegate: EmbyImagePipelineDelegate()) { config in
        config.dataCache = DataCache.Emby.local
...
final class EmbyImagePipelineDelegate: ImagePipeline.Delegate {
