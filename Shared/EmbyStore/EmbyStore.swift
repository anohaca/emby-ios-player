...
typealias AnyStoredData = EmbyStore.V3.AnyData

typealias ServerState = EmbyStore.State.Server
typealias UserState = EmbyStore.State.User
...
extension EmbyStore {
    ...
    private static let storage: SQLiteStore = {
        SQLiteStore(
            fileName: "Emby.sqlite"
        )
    }()

    private static let appOwnerID = "embyApp"
    ...
    private static func loadV1Snapshots(
        ...
        let servers = try sourceStack.fetchAll(From<EmbyStore.V1.StoredServer>())
            .map {
                V1ServerSnapshot(
                    id: $0.id,
                    name: $0.name,
        ...
        let users = try sourceStack.fetchAll(From<EmbyStore.V1.StoredUser>())
            .map {
                V1UserSnapshot(
                    id: $0.id,
                    username: $0.username,
    ...
    private static func createV2Store(
        ...
        try destinationStack.perform { transaction in
            var v2ServersByID: [String: EmbyStore.V2.StoredServer] = [:]
            for server in servers {
                let newServer = transaction.create(Into<EmbyStore.V2.StoredServer>())
            ...
            for user in users {
                let newUser = transaction.create(Into<EmbyStore.V2.StoredUser>())
    ...
    private static func loadV2AnyDataSnapshots(from sourceStoreURL: URL) async throws -> [V2AnyDataSnapshot] {
        ...
        return try sourceStack.fetchAll(From<EmbyStore.V2.AnyData>())
            .map {
                V2AnyDataSnapshot(
                    data: $0.data,
                    ownerID: $0.ownerID,
    ...
    private static func createV3Store(
        ...
        try destinationStack.perform { transaction in
            func upsertAnyData(ownerID: String, field: String, key: String, data: Data?) throws {
                let ownerFilter: Where<EmbyStore.V3.AnyData> = Where(\.$ownerID == ownerID)
                let fieldFilter: Where<EmbyStore.V3.AnyData> = Where(\.$field == field)
                let keyFilter: Where<EmbyStore.V3.AnyData> = Where(\.$key == key)
                let existing = try transaction.fetchOne(
                    From<EmbyStore.V3.AnyData>()
                ...
                if let existing {
                    ...
                    let newEntry = transaction.create(Into<EmbyStore.V3.AnyData>())
    ...
    private static func loadV2LegacyStates(
        ...
        let servers = try sourceStack.fetchAll(From<EmbyStore.V2.StoredServer>()).map(\.state)
        let users = try sourceStack.fetchAll(From<EmbyStore.V2.StoredUser>()).map(\.state)
